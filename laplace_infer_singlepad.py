"""
对 singlepad/singlepinpad 模型做：last-layer Laplace Approximation + 指标&校准评估

流程：
1. 读取 CSV，利用其中的软标签列：
   - NONE_CONF / INSUFFICIENT_SOLDER_CONF / PSEUDO_SOLDER_CONF
     的 argmax 作为 3 类 ground truth（0=ok,1=undersolder,2=pseudosolder）

2. 用这些“软标签 argmax”在全量样本上拟合 BOM 头最后一层 Linear 的 Laplace 后验
   - 只对 BOM 最后一层 Linear 做 Laplace（last-layer）
   - 用经验 Fisher: E[g^2] 估计 Hessian 对角线

3. 推理阶段：
   - 对该层参数 (W,b) 做高斯采样，多次 forward
   - 对每一类概率取均值 & 标准差，按 id 写回 CSV
   - 列名为：
       NONE_CONF_LAPLACE
       INSUFFICIENT_SOLDER_CONF_LAPLACE
       PSEUDO_SOLDER_CONF_LAPLACE
       以及对应的 *_STD

4. 写回 CSV 后，在本脚本内做评估：
   - 在 threshold = 0.5 / 0.8 下分别输出：
       binary accuracy  (defect vs ok)
       mclass accuracy  (3-class)
       == Binary output ==
           weighted f1 score
           ok / defect 的 precision / recall / f1_score / omission_rate
       == Multiclass output ==
           weighted f1 score
           ok / undersolder / pseudosolder 的 precision / recall / f1_score / omission_rate

   - 校准指标（ECE），和 deepensemble 版本保持一致：
       Binary：
         - overall ECE（top1）
         - true=ok / true=defect 的 prob-wise classwise ECE
       Multiclass：
         - overall ECE（top1）
         - true=ok / true=undersolder / true=pseudosolder 的 prob-wise classwise ECE
         - Multiclass vector ECE：每个样本所有类概率都参与

   - 可靠性图（reliability diagram）：
       Binary：
         - overall（top1：ok vs defect）一张
         - true=ok 一张
         - true=defect 一张
       Multiclass：
         - overall（top1：3 类）一张
         - true=ok / true=undersolder / true=pseudosolder 各一张
         - Multiclass vector 版：gap 柱形图 + 每个类一张向量版 reliability
       * 都是柱状图展示 bin 的 accuracy + y=x 参考线
"""

import os
import glob
import numpy as np
import pandas as pd

import torch
import torch.nn as nn
import torch.nn.functional as F

from tqdm.auto import tqdm
import matplotlib
matplotlib.use("Agg")  # 服务器环境避免 GUI
import matplotlib.pyplot as plt

# 注意：请确保 utils.utilities 和 models.MPB3net 能正确导入
from utils.utilities import TransformImage
from models.MPB3 import MPB3net


# ==============================
# 基本配置：路径 & 模型结构
# ==============================

CKPT_DIR = "/home/cat/workspace/vlm/scripts/models/checkpoints"

N_CLASS = 3
N_CLASSES = 3
DEVICE = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

ROOT_DIR = (
    "/home/cat/workspace/defect_data/"
    "defect_DA758_black_uuid_250310/send2terminal/250310"
)
CSV_PATH = os.path.join(ROOT_DIR, "checked_samples.csv")

CLASS_ORDER = ["ok", "undersolder", "pseudosolder"]

PART_CONFIG = {
    "singlepad": {
        "ckpt_pattern": "singlepadfcdropoutmobilenetv3largers6464*dual2*.pth.tar",
        "backbone": "fcdropoutmobilenetv3large",
        "n_units": [256, 256],
        "n_class": N_CLASS,
        "img_h": 64,
        "img_w": 64,
    },
    "singlepinpad": {
        "ckpt_pattern": "singlepinpadmobilenetv3smallrs12832*dual2*.pth.tar",
        "backbone": "mobilenetv3small",
        "n_units": [256, 256],
        "n_class": N_CLASS,
        "img_h": 32,
        "img_w": 128,
    },
}

# ==============================
# 评估相关的超参数 / 常量
# ==============================

# F1 曲线的阈值个数
N_THRESH = 101

# 在这些阈值下打印详细的二/三分类指标
THRESH_LIST = [0.5, 0.8]

# ECE bin 个数
N_BINS_ECE = 15

# 评估图保存目录
SAVE_DIR = os.path.join(ROOT_DIR, "laplace_eval")
os.makedirs(SAVE_DIR, exist_ok=True)

# 使用英文字体
plt.rcParams["font.family"] = "DejaVu Sans"


# ==============================
# 通用：权重加载辅助函数
# ==============================

def load_model_by_values(ckp: dict, model: nn.Module) -> None:
    """当 ckpt 的 key 和 model.state_dict 的 key 对不上时，按顺序依次把 value 填到模型里。"""
    model_state_dict = model.state_dict()
    model_state_dict_keys = list(model_state_dict.keys())
    index = 0
    new_state_dict = {}

    for key, value in ckp.items():
        if index >= len(model_state_dict_keys):
            print(f"[WARN] ckpt extra param: {key}")
            break

        model_key = model_state_dict_keys[index]

        if key == model_key and model_state_dict[model_key].shape == value.shape:
            new_state_dict[model_key] = value
        elif model_state_dict[model_key].shape == value.shape:
            print(f"ckp {key} -> model {model_key}")
            new_state_dict[model_key] = value
        else:
            print(
                f"Key mismatch: ckpt {key} ({value.shape}) "
                f"vs model {model_key} ({model_state_dict[model_key].shape})"
            )

        index += 1

    model.load_state_dict(new_state_dict, strict=True)


def _find_ckpt(pattern: str) -> str:
    """在 CKPT_DIR 下用通配符找 checkpoint 文件（目前固定用指定的那一个 ckpt）。"""
    full_pattern = os.path.join(CKPT_DIR, pattern)
    files = glob.glob(full_pattern)
    if not files:
        raise FileNotFoundError(f"ckpt not found: {full_pattern}")
    files = sorted(files)

    ckpt_path = (
        "/home/cat/workspace/vlm/scripts/models/checkpoints/singlepadfcdropoutmobilenetv3largers6464s42c3val0.1b256_ckp_v0.18.9lhf1certainlut05cp05clean20.0j0.4lr0.025nb256nm256dual2top2.pth.tar"
    )
    print(f"[INFO] found {len(files)} ckpt(s), use: {ckpt_path}")
    return ckpt_path


def _build_model_for_part(part_name: str) -> MPB3net:
    """构建并加载 singlepad / singlepinpad 模型。"""
    if part_name not in PART_CONFIG:
        raise ValueError(f"unknown part_name: {part_name}")

    cfg = PART_CONFIG[part_name]
    ckpt_path = _find_ckpt(cfg["ckpt_pattern"])

    print(f"=> loading CNN for {part_name}: {ckpt_path}")

    model = MPB3net(
        backbone=cfg["backbone"],
        pretrained=False,
        n_class=cfg["n_class"],
        n_units=cfg["n_units"],
        output_form="dual2",
    )

    ckpt = torch.load(ckpt_path, map_location="cpu")
    if "state_dict" in ckpt:
        state_dict = ckpt["state_dict"]
    else:
        state_dict = ckpt

    try:
        model.load_state_dict(state_dict, strict=True)
    except Exception as e:
        print(f"[WARN] strict=True failed, try value-align: {e}")
        load_model_by_values(state_dict, model)

    model.to(DEVICE)
    model.eval()
    print(f"=> {part_name} model loaded on {DEVICE}")
    return model


# ==============================
# Laplace 相关：找 BOM 头、遍历数据
# ==============================

def _find_bom_last_layer(model: nn.Module, n_class: int) -> nn.Linear:
    """在整个模型里搜索 out_features == n_class 的 nn.Linear，把最后一个当作 BOM 头。"""
    candidates = []
    for m in model.modules():
        if isinstance(m, nn.Linear) and m.out_features == n_class:
            candidates.append(m)

    if not candidates:
        raise RuntimeError(f"no nn.Linear with out_features={n_class} found")

    if len(candidates) > 1:
        print("[WARN] multiple candidate BOM heads, use the last one:")
        for idx, layer in enumerate(candidates):
            print(
                f"   [{idx}] in_features={layer.in_features}, "
                f"out_features={layer.out_features}"
            )

    bom_layer = candidates[-1]
    print(
        f"[INFO] BOM head Linear: "
        f"in_features={bom_layer.in_features}, out_features={bom_layer.out_features}"
    )
    return bom_layer


def _iter_images_from_csv(
    csv_path: str,
    root_path: str,
    part_name: str,
    batch_size: int,
    use_soft_labels: bool = False,
):
    """
    一个 generator：
    - 按 batch 从 CSV 中读取图片，返回：
        x1: [B, 3, H, W]
        x2: [B, 3, H, W]
        y:  若 use_soft_labels=True，则是 soft label argmax 得到的 [B] LongTensor
        meta_list: 每张图的 meta 信息（id / ref_path / insp_path）
    """
    cfg = PART_CONFIG[part_name]
    df = pd.read_csv(csv_path)

    if "part_name" in df.columns:
        df = df[df["part_name"] == part_name].reset_index(drop=True)

    num_samples = len(df)

    for start in range(0, num_samples, batch_size):
        end = min(start + batch_size, num_samples)
        batch_df = df.iloc[start:end]

        img1_list, img2_list, meta_list, label_list = [], [], [], []

        for _, row in batch_df.iterrows():
            sample_id = row["id"]
            ref_rel = row["ref_image"]
            insp_rel = row["insp_image"]

            ref_path = os.path.join(root_path, ref_rel)
            insp_path = os.path.join(root_path, insp_rel)

            ref_img = TransformImage(
                img_path=ref_path,
                rs_img_size_h=cfg["img_h"],
                rs_img_size_w=cfg["img_w"],
            ).transform()

            insp_img = TransformImage(
                img_path=insp_path,
                rs_img_size_h=cfg["img_h"],
                rs_img_size_w=cfg["img_w"],
            ).transform()

            ref_tensor = torch.FloatTensor(ref_img)
            insp_tensor = torch.FloatTensor(insp_img)

            img1_list.append(ref_tensor)
            img2_list.append(insp_tensor)

            meta_list.append({
                "id": sample_id,
                "ref_path": ref_rel,
                "insp_path": insp_rel,
            })

            if use_soft_labels:
                soft = np.array(
                    [
                        row["NONE_CONF"],
                        row["INSUFFICIENT_SOLDER_CONF"],
                        row["PSEUDO_SOLDER_CONF"],
                    ],
                    dtype=np.float32,
                )
                label_idx = int(soft.argmax())  # 0/1/2
                label_list.append(label_idx)

        x1 = torch.cat(img1_list, dim=0).to(DEVICE)
        x2 = torch.cat(img2_list, dim=0).to(DEVICE)

        if use_soft_labels:
            y = torch.LongTensor(label_list).to(DEVICE)
        else:
            y = None

        yield x1, x2, y, meta_list, len(batch_df), num_samples


# ==============================
# 拟合 Laplace：经验 Fisher + 先验
# ==============================

def fit_laplace_last_layer_bom(
    model: nn.Module,
    csv_path: str,
    root_path: str,
    part_name: str = "singlepad",
    batch_size: int = 64,
    prior_precision: float = 1.0,
    fisher_scale: float = 1.0,
):
    """
    使用 CSV 中的软标签拟合 BOM 头最后一层 Linear 的 Laplace 后验：
    - 只对 BOM 最后一层做 Laplace（last-layer）
    - 标签来自 soft label argmax
    - 使用经验 Fisher：E[g^2] 估计 Hessian 对角
    """
    cfg = PART_CONFIG[part_name]

    bom_layer = _find_bom_last_layer(model, cfg["n_class"])

    for p in model.parameters():
        p.requires_grad_(False)
    for p in bom_layer.parameters():
        p.requires_grad_(True)

    fisher_w = torch.zeros_like(bom_layer.weight, device=DEVICE)
    fisher_b = torch.zeros_like(bom_layer.bias, device=DEVICE)
    n_batches = 0

    model.eval()

    total_imgs = len(pd.read_csv(csv_path))
    print("[*] start Laplace fitting (last-layer, empirical Fisher)")
    with tqdm(total=total_imgs, desc=f"{part_name} Laplace-fit", unit="img") as pbar:
        for x1, x2, y, meta_list, batch_len, num_samples in _iter_images_from_csv(
            csv_path,
            root_path,
            part_name,
            batch_size,
            use_soft_labels=True,
        ):
            _, logits_bom = model(x1, x2)

            log_probs = F.log_softmax(logits_bom, dim=-1)
            nll = F.nll_loss(log_probs, y, reduction="mean")

            model.zero_grad(set_to_none=True)
            nll.backward()

            fisher_w += bom_layer.weight.grad.detach() ** 2
            fisher_b += bom_layer.bias.grad.detach() ** 2

            n_batches += 1
            pbar.update(batch_len)

    fisher_w /= max(n_batches, 1)
    fisher_b /= max(n_batches, 1)

    precision_w = prior_precision + fisher_scale * fisher_w
    precision_b = prior_precision + fisher_scale * fisher_b

    var_w = 1.0 / (precision_w + 1e-8)
    var_b = 1.0 / (precision_b + 1e-8)

    laplace_state = {
        "mean_weight": bom_layer.weight.detach().clone(),
        "mean_bias": bom_layer.bias.detach().clone(),
        "var_weight": var_w.detach().clone(),
        "var_bias": var_b.detach().clone(),
    }

    for p in model.parameters():
        p.requires_grad_(False)

    print("[*] Laplace fitting done (using CSV soft-label argmax)")
    return bom_layer, laplace_state


# ==============================
# 推理：对 BOM 头做参数采样
# ==============================

@torch.no_grad()
def laplace_predict_bos_bom(
    model: nn.Module,
    bom_layer: nn.Linear,
    laplace_state: dict,
    x1: torch.Tensor,
    x2: torch.Tensor,
    mc_samples: int = 50,
):
    """使用 Laplace 后验对 (W,b) 采样，多次 forward，返回 softmax 概率的均值 & 标准差。"""
    w_mean = laplace_state["mean_weight"].to(DEVICE)
    b_mean = laplace_state["mean_bias"].to(DEVICE)
    w_std = laplace_state["var_weight"].to(DEVICE).clamp(min=1e-12).sqrt()
    b_std = laplace_state["var_bias"].to(DEVICE).clamp(min=1e-12).sqrt()

    bom_layer.weight.data = w_mean.clone()
    bom_layer.bias.data = b_mean.clone()

    probs_list = []

    for _ in range(mc_samples):
        eps_w = torch.randn_like(w_mean)
        eps_b = torch.randn_like(b_mean)

        sampled_w = w_mean + w_std * eps_w
        sampled_b = b_mean + b_std * eps_b

        bom_layer.weight.data = sampled_w
        bom_layer.bias.data = sampled_b

        _, logits_bom = model(x1, x2)
        probs_bom = F.softmax(logits_bom, dim=-1)
        probs_list.append(probs_bom)

    # 还原为 mean 权重
    bom_layer.weight.data = w_mean.clone()
    bom_layer.bias.data = b_mean.clone()

    probs_arr = torch.stack(probs_list, dim=0)  # [S, B, C]
    bom_mean = probs_arr.mean(dim=0)
    bom_std = probs_arr.std(dim=0)

    return {
        "bom_mean": bom_mean,
        "bom_std": bom_std,
    }


# ==============================
# 批量推理 + 写回 CSV
# ==============================

def infer_singlepad_batch_laplace(
    csv_path: str,
    root_path: str,
    model: MPB3net,
    bom_layer: nn.Linear,
    laplace_state: dict,
    part_name: str = "singlepad",
    batch_size: int = 32,
    mc_samples: int = 50,
) -> pd.DataFrame:
    """对 CSV 中该 part 的样本做 Laplace 推理，返回包含 id 的 DataFrame。"""
    df = pd.read_csv(csv_path)

    if "part_name" in df.columns:
        df = df[df["part_name"] == part_name].reset_index(drop=True)

    results = []
    num_samples = len(df)
    print(
        f"[*] {num_samples} samples, "
        f"start Laplace inference (batch_size={batch_size}, mc_samples={mc_samples})"
    )

    with tqdm(total=num_samples, desc=f"{part_name} Laplace-predict", unit="img") as pbar:
        for start in range(0, num_samples, batch_size):
            end = min(start + batch_size, num_samples)
            batch_df = df.iloc[start:end]

            img1_list, img2_list = [], []
            meta_list = []

            cfg = PART_CONFIG[part_name]

            for _, row in batch_df.iterrows():
                sample_id = row["id"]
                ref_rel = row["ref_image"]
                insp_rel = row["insp_image"]

                ref_path = os.path.join(root_path, ref_rel)
                insp_path = os.path.join(root_path, insp_rel)

                ref_img = TransformImage(
                    img_path=ref_path,
                    rs_img_size_h=cfg["img_h"],
                    rs_img_size_w=cfg["img_w"],
                ).transform()

                insp_img = TransformImage(
                    img_path=insp_path,
                    rs_img_size_h=cfg["img_h"],
                    rs_img_size_w=cfg["img_w"],
                ).transform()

                ref_tensor = torch.FloatTensor(ref_img)
                insp_tensor = torch.FloatTensor(insp_img)

                img1_list.append(ref_tensor)
                img2_list.append(insp_tensor)

                meta_list.append({
                    "id": sample_id,
                    "ref_path": ref_rel,
                    "insp_path": insp_rel,
                })

            x1 = torch.cat(img1_list, dim=0).to(DEVICE)
            x2 = torch.cat(img2_list, dim=0).to(DEVICE)

            lap_out = laplace_predict_bos_bom(
                model,
                bom_layer,
                laplace_state,
                x1,
                x2,
                mc_samples=mc_samples,
            )
            bom_mean = lap_out["bom_mean"]
            bom_std = lap_out["bom_std"]

            bom_mean_np = bom_mean.cpu().numpy()
            bom_std_np = bom_std.cpu().numpy()

            for i, meta in enumerate(meta_list):
                probs = bom_mean_np[i]
                stds = bom_std_np[i]

                row_result = {
                    **meta,
                    "NONE_CONF_LAPLACE":                    float(probs[0]),
                    "INSUFFICIENT_SOLDER_CONF_LAPLACE":     float(probs[1]),
                    "PSEUDO_SOLDER_CONF_LAPLACE":           float(probs[2]),
                    "NONE_CONF_LAPLACE_STD":                float(stds[0]),
                    "INSUFFICIENT_SOLDER_CONF_LAPLACE_STD": float(stds[1]),
                    "PSEUDO_SOLDER_CONF_LAPLACE_STD":       float(stds[2]),
                }
                results.append(row_result)

            pbar.update(len(batch_df))

    result_df = pd.DataFrame(results)

    cols = [
        "id",
        "ref_path",
        "insp_path",
        "NONE_CONF_LAPLACE",
        "INSUFFICIENT_SOLDER_CONF_LAPLACE",
        "PSEUDO_SOLDER_CONF_LAPLACE",
        "NONE_CONF_LAPLACE_STD",
        "INSUFFICIENT_SOLDER_CONF_LAPLACE_STD",
        "PSEUDO_SOLDER_CONF_LAPLACE_STD",
    ]
    result_df = result_df[cols]
    return result_df


# ==============================
# 评估工具：混淆矩阵 & F1
# ==============================

def confusion_matrix_3cls(
    y_true: np.ndarray,
    y_pred: np.ndarray,
    n_class: int = N_CLASSES,
):
    """cm[i, j] = 真实为 i, 预测为 j 的样本数。"""
    cm = np.zeros((n_class, n_class), dtype=np.int64)
    for t, p in zip(y_true, y_pred):
        cm[int(t), int(p)] += 1
    return cm


def calc_precision_recall_f1_binary(
    y_true_bin: np.ndarray,
    scores_pos: np.ndarray,
    thresholds: np.ndarray,
):
    """二分类（正类=defect）的 P/R/F1 曲线。"""
    precisions, recalls, f1s = [], [], []

    for thr in thresholds:
        y_pred_bin = (scores_pos >= thr).astype(np.int64)

        tp = int(((y_pred_bin == 1) & (y_true_bin == 1)).sum())
        fp = int(((y_pred_bin == 1) & (y_true_bin == 0)).sum())
        fn = int(((y_pred_bin == 0) & (y_true_bin == 1)).sum())

        if tp + fp == 0:
            prec = 1.0
        else:
            prec = tp / (tp + fp)

        if tp + fn == 0:
            rec = 0.0
        else:
            rec = tp / (tp + fn)

        if prec + rec == 0:
            f1 = 0.0
        else:
            f1 = 2 * prec * rec / (prec + rec)

        precisions.append(prec)
        recalls.append(rec)
        f1s.append(f1)

    return (
        np.array(precisions, dtype=np.float32),
        np.array(recalls, dtype=np.float32),
        np.array(f1s, dtype=np.float32),
        thresholds,
    )


def metrics_from_cm(cm: np.ndarray):
    """
    从混淆矩阵 cm 计算：
      - 每个类别：precision / recall / f1
      - 每个类别 support
      - weighted F1（按 support 加权）
    """
    C = cm.shape[0]
    per_prec = np.zeros(C, dtype=float)
    per_rec = np.zeros(C, dtype=float)
    per_f1 = np.zeros(C, dtype=float)
    supports = cm.sum(axis=1)
    total_support = supports.sum()

    weighted_f1_num = 0.0

    for c in range(C):
        tp = cm[c, c]
        support = supports[c]
        pred_pos = cm[:, c].sum()

        if support == 0:
            rec = float("nan")
        else:
            rec = tp / support

        if pred_pos == 0:
            prec = float("nan")
        else:
            prec = tp / pred_pos

        if np.isnan(prec) or np.isnan(rec) or prec + rec == 0:
            f1 = float("nan")
        else:
            f1 = 2 * prec * rec / (prec + rec)

        per_prec[c], per_rec[c], per_f1[c] = prec, rec, f1

        if not np.isnan(f1):
            weighted_f1_num += f1 * support

    weighted_f1 = (
        weighted_f1_num / total_support if total_support > 0 else float("nan")
    )

    return per_prec, per_rec, per_f1, supports, weighted_f1


# ==============================
# ECE & 可靠性图（标量事件版）
# ==============================

def compute_ece(confidences: np.ndarray,
                correctness: np.ndarray,
                n_bins: int = N_BINS_ECE):
    """
    标准 ECE 计算:
      - confidences: [N], 某个“事件”的预测概率（0~1）
      - correctness: [N], 0/1 该事件是否发生

    返回:
      ece, bin_edges, bin_conf_mean, bin_acc_mean, bin_counts
    """
    conf = np.asarray(confidences, dtype=np.float64)
    corr = np.asarray(correctness, dtype=np.float64)

    conf = np.clip(conf, 0.0, 1.0)
    assert conf.shape == corr.shape

    bin_edges = np.linspace(0.0, 1.0, n_bins + 1)
    bin_conf = np.zeros(n_bins, dtype=float)
    bin_acc = np.zeros(n_bins, dtype=float)
    bin_counts = np.zeros(n_bins, dtype=int)

    # digitize: 返回 bin index (0..n_bins-1)
    bin_ids = np.digitize(conf, bin_edges[1:-1], right=True)

    for b in range(n_bins):
        mask = (bin_ids == b)
        cnt = int(mask.sum())
        bin_counts[b] = cnt
        if cnt > 0:
            bin_conf[b] = float(conf[mask].mean())
            bin_acc[b] = float(corr[mask].mean())
        else:
            bin_conf[b] = 0.0
            bin_acc[b] = 0.0

    N = conf.shape[0]
    ece = 0.0
    for b in range(n_bins):
        if bin_counts[b] == 0:
            continue
        weight = bin_counts[b] / N
        ece += weight * abs(bin_acc[b] - bin_conf[b])

    return ece, bin_edges, bin_conf, bin_acc, bin_counts


def plot_reliability_bars(
    bin_edges: np.ndarray,
    bin_acc: np.ndarray,
    title: str,
    out_path: str,
):
    """
    按 bin 画柱状可靠性图：
      - x 轴：bin 区间
      - y 轴：该 bin 内的 empirical accuracy
      - 叠加一条 y=x 虚线作为 perfect calibration
    """
    n_bins = len(bin_acc)
    width = bin_edges[1] - bin_edges[0]
    centers = bin_edges[:-1] + width / 2.0

    plt.figure(figsize=(5, 5))

    # 柱状：每个 bin 的 accuracy
    plt.bar(
        centers,
        bin_acc,
        width=width * 0.9,
        align="center",
        alpha=0.7,
        edgecolor="black",
        label="Empirical accuracy",
    )

    # 理想校准线
    plt.plot([0, 1], [0, 1], "--", color="gray", label="Perfect calibration")

    plt.xlabel("Predicted probability")
    plt.ylabel("Empirical accuracy")
    plt.title(title)
    plt.grid(True, linestyle="--", alpha=0.3)
    plt.xlim(0.0, 1.0)
    plt.ylim(0.0, 1.0)
    plt.legend(loc="best")

    plt.tight_layout()
    plt.savefig(out_path, dpi=200, bbox_inches="tight")
    plt.close()
    print(f"[INFO] reliability diagram saved to: {out_path}")


# ==============================
# Multiclass “向量版” ECE 及绘图
# ==============================

def compute_multiclass_vector_ece(
    softmaxes: np.ndarray,
    labels: np.ndarray,
    n_bins: int = N_BINS_ECE,
):
    """
    Multiclass 向量版 ECE（按 max 置信度分 bin + 按预测类别细分）:

    对每个样本 i:
      - probs[i, :] 为 softmax 概率
      - pred[i] = argmax_k probs[i, k] 为预测类别
      - max_conf[i] = probs[i, pred[i]] 为最大置信度

    步骤:
      1) 按 max_conf 分成若干个置信度区间（bin）。
      2) 在每个 bin 中，再按预测类别 pred 分成 C 组。
      3) 对每个 (bin b, 类别 k):
           acc_{b,k}  = 该组中 true label == k 的比例
           conf_{b,k} = 该组中 probs[:, k] 的平均值 (等同于 max_conf)
           gap_{b,k}  = |acc_{b,k} - conf_{b,k}|
      4) 全局 ECE:
           ECE = Σ_{b,k} (|S_{b,k}| / N) * gap_{b,k}
         其中 S_{b,k} 是 bin b 中预测为 k 的样本集合。
      5) per-bin gap:
           g_b = Σ_k (|S_{b,k}| / |S_b|) * gap_{b,k}
         MCE 取 max_b g_b。

    返回:
        ece:        标量，全局 ECE
        mce:        标量，最大 per-bin gap
        bin_edges:  [n_bins+1]，bin 边界
        bin_counts: [n_bins]，每个 bin 的样本数 |S_b|
        gap_per_bin:[n_bins]，每个 bin 的 g_b
        acc_means:  [C, n_bins]，acc_{b,k}（没样本时为 NaN）
        conf_means: [C, n_bins]，conf_{b,k}（没样本时为 NaN）
    """
    probs = np.asarray(softmaxes, dtype=np.float64)   # [N, C]
    labels = np.asarray(labels, dtype=np.int64)       # [N]
    N, C = probs.shape

    # top-1 预测类别 & 对应最大置信度
    pred = probs.argmax(axis=1)                       # [N]
    max_conf = probs[np.arange(N), pred]             # [N]

    # 按 max_conf 等宽分 bin
    bin_edges = np.linspace(0.0, 1.0, n_bins + 1)
    bin_lowers = bin_edges[:-1]
    bin_uppers = bin_edges[1:]

    # 结果数组
    acc_means = np.full((C, n_bins), np.nan, dtype=np.float64)
    conf_means = np.full((C, n_bins), np.nan, dtype=np.float64)
    bin_counts = np.zeros(n_bins, dtype=int)
    gap_per_bin = np.zeros(n_bins, dtype=np.float64)

    ece = 0.0
    mce = 0.0

    for b, (lower, upper) in enumerate(zip(bin_lowers, bin_uppers)):
        # 当前 bin 内的样本
        in_bin = (max_conf > lower) & (max_conf <= upper)
        count_b = int(in_bin.sum())
        bin_counts[b] = count_b
        if count_b == 0:
            continue

        # 该 bin 内的“局部 ECE”
        g_b = 0.0

        # 在 bin 内再按预测类别分组
        for k in range(C):
            in_bin_k = in_bin & (pred == k)
            count_bk = int(in_bin_k.sum())
            if count_bk == 0:
                continue

            # acc_{b,k}：这一组里预测对的比例
            acc_k = (labels[in_bin_k] == k).mean()

            # conf_{b,k}：这一组里对类别 k 的平均置信度
            # 因为 pred == k，这里用 probs[:, k] 或 max_conf 都一样
            conf_k = probs[in_bin_k, k].mean()

            acc_means[k, b] = acc_k
            conf_means[k, b] = conf_k

            gap = abs(acc_k - conf_k)

            # 组内权重：该组在当前 bin 内的占比
            weight_in_bin = count_bk / count_b
            g_b += weight_in_bin * gap

            # 全局 ECE 的权重：该组在全体样本中的占比
            ece += (count_bk / N) * gap

        gap_per_bin[b] = g_b
        if g_b > mce:
            mce = g_b

    return ece, mce, bin_edges, bin_counts, gap_per_bin, acc_means, conf_means


def plot_multiclass_vector_gap(
    bin_edges: np.ndarray,
    gap_per_bin: np.ndarray,
    title: str,
    out_path: str,
):
    """
    画每个 bin 的向量 gap g_m 的柱形图。
    """
    width = bin_edges[1] - bin_edges[0]
    centers = bin_edges[:-1] + width / 2.0

    plt.figure(figsize=(6, 4))
    plt.bar(
        centers,
        gap_per_bin,
        width=width * 0.9,
        align="center",
        alpha=0.7,
        edgecolor="black",
        label=r"$\sum_k |acc_k - conf_k|$",
    )

    plt.xlabel("Max predicted probability bin")
    plt.ylabel("Per-bin vector gap")
    plt.title(title)
    plt.grid(True, linestyle="--", alpha=0.3)
    plt.xlim(0.0, 1.0)

    plt.tight_layout()
    plt.savefig(out_path, dpi=200, bbox_inches="tight")
    plt.close()
    print(f"[INFO] multiclass vector gap diagram saved to: {out_path}")


def plot_multiclass_vector_classwise(
    bin_edges: np.ndarray,
    acc_means: np.ndarray,
    conf_means: np.ndarray,
    class_idx: int,
    class_name: str,
    title_prefix: str,
    out_path: str,
):
    """
    对某一个类别画“向量版 per-class gap 图”：
      - x 轴：max_conf 的 bin 中心
      - 柱形：该 bin 内该类的 gap = |acc_k - conf_k|
        （acc_k: 该 bin 内 true=class_k 的频率；
         conf_k: 该 bin 内该类预测概率的平均值）
    """
    acc = acc_means[class_idx]    # [n_bins]
    conf = conf_means[class_idx]  # [n_bins]
    gap = np.abs(acc - conf)      # [n_bins]

    n_bins = len(gap)
    width = bin_edges[1] - bin_edges[0]
    centers = bin_edges[:-1] + width / 2.0

    # 没有样本的 bin 会是 NaN，过滤掉
    mask = ~np.isnan(gap)

    plt.figure(figsize=(5, 5))
    plt.bar(
        centers[mask],
        gap[mask],
        width=width * 0.9,
        align="center",
        alpha=0.7,
        edgecolor="black",
        label=r"|acc - conf| (class=%s)" % class_name,
    )

    plt.xlabel("Max predicted probability bin")
    plt.ylabel(r"Per-bin gap |acc - conf|")
    plt.title(f"{title_prefix} (class={class_name})")
    plt.grid(True, linestyle="--", alpha=0.3)
    plt.xlim(0.0, 1.0)
    plt.ylim(0.0, 1.0)  # 对于单类 gap, 取值范围 [0,1]
    plt.legend(loc="best")

    plt.tight_layout()
    plt.savefig(out_path, dpi=200, bbox_inches="tight")
    plt.close()
    print(
        f"[INFO] multiclass vector per-class gap "
        f"(class={class_name}) saved to: {out_path}"
    )


# ==============================
# 指标评估：阈值 + ECE + 多张可靠性图
# ==============================

def eval_singlepad_by_threshold(
    csv_path: str,
    save_dir: str,
    part_name: str = "singlepad",
):
    """
    仿照 deepensemble 版本输出二/三分类指标，并计算 ECE + 画可靠性图：

    Binary：
      - overall ECE（top1）
      - true=ok / true=defect 的 prob-wise classwise ECE

    Multiclass：
      - overall ECE（top1）
      - true=ok / true=undersolder / true=pseudosolder 的 prob-wise classwise ECE
      - Multiclass vector ECE：每个样本所有类概率都参与
    """

    df = pd.read_csv(csv_path)

    if "part_name" in df.columns:
        df = df[df["part_name"] == part_name].reset_index(drop=True)

    if len(df) == 0:
        print(f"[WARN] no data for part_name={part_name}, skip eval")
        return

    # === 1) ground truth ===
    gt_cols = ["NONE_CONF", "INSUFFICIENT_SOLDER_CONF", "PSEUDO_SOLDER_CONF"]
    if not all(col in df.columns for col in gt_cols):
        raise ValueError(
            "CSV missing ground truth columns: "
            "NONE_CONF / INSUFFICIENT_SOLDER_CONF / PSEUDO_SOLDER_CONF"
        )

    soft_gt = df[gt_cols].to_numpy(dtype=np.float32)  # [N, 3]
    y_true_3 = soft_gt.argmax(axis=1).astype(np.int64)  # 0/1/2
    y_true_bin = (y_true_3 != 0).astype(np.int64)       # defect = 非 ok

    # === 2) 只评估 Laplace ===
    methods = {
        "Laplace": {
            "cols": [
                "NONE_CONF_LAPLACE",
                "INSUFFICIENT_SOLDER_CONF_LAPLACE",
                "PSEUDO_SOLDER_CONF_LAPLACE",
            ],
            "color": "C2",
            "marker": "s",
        },
    }

    thresholds_curve = np.linspace(0.0, 1.0, N_THRESH)
    f1_curves = {}

    print("\n===== singlepad Laplace evaluation (3-class & binary + calibration) =====")

    for method_name, cfg in methods.items():
        prob_cols = cfg["cols"]
        if not all(col in df.columns for col in prob_cols):
            print(f"[WARN] skip {method_name}, missing cols: {prob_cols}")
            continue

        probs = df[prob_cols].to_numpy(dtype=np.float32)  # [N, 3]

        # ---------- binary: defect vs ok ----------
        p_ok = probs[:, 0]
        p_defect = 1.0 - p_ok
        scores_defect = p_defect

        # F1 曲线（defect = 正类）
        _, _, f1s_curve, _ = calc_precision_recall_f1_binary(
            y_true_bin, scores_defect, thresholds_curve
        )
        f1_curves[method_name] = {
            "f1": f1s_curve,
            "color": cfg["color"],
            "marker": cfg["marker"],
        }

        # ====================== 核心修改：阈值循环内重新计算多分类指标 ======================
        for thr in THRESH_LIST:
            print(f"\nThreshold = {thr:.2f}  --  [{method_name}]")
            print(f"---- threshold = {thr:.2f} ----")

            # ---------------- 1. 二分类：按阈值计算，新增TP/FP/FN打印 ----------------
            y_pred_bin = (scores_defect >= thr).astype(np.int64)
            acc_bin = (y_pred_bin == y_true_bin).mean()

            # ADD: 打印二分类原始TP/FP/FN/TN
            TP = int(((y_pred_bin == 1) & (y_true_bin == 1)).sum())
            FP = int(((y_pred_bin == 1) & (y_true_bin == 0)).sum())
            FN = int(((y_pred_bin == 0) & (y_true_bin == 1)).sum())
            TN = int(((y_pred_bin == 0) & (y_true_bin == 0)).sum())


            cm_bin = confusion_matrix_3cls(y_true_bin, y_pred_bin, n_class=2)
            per_prec_bin, per_rec_bin, per_f1_bin, support_bin, weighted_f1_bin = metrics_from_cm(cm_bin)

            # ---------------- 2. 多分类：按阈值重新计算（关键修复） ----------------
            # 多分类阈值逻辑：仅当最大概率≥阈值时，才预测对应类别；否则标记为未知(-1)
            max_probs_multi = np.max(probs, axis=1)
            y_pred_3 = np.where(max_probs_multi >= thr, probs.argmax(axis=1), -1)  # -1=未知

            # 过滤未知样本，只计算有效预测
            valid_mask = y_pred_3 != -1
            if np.sum(valid_mask) == 0:
                print("  [多分类] 所有样本概率低于阈值，无有效预测")
                acc_mclass = 0.0
                weighted_f1_multi = 0.0
                per_prec_multi = np.zeros(N_CLASSES)
                per_rec_multi = np.zeros(N_CLASSES)
                per_f1_multi = np.zeros(N_CLASSES)
                support_multi = np.zeros(N_CLASSES)
            else:
                y_true_3_valid = y_true_3[valid_mask]
                y_pred_3_valid = y_pred_3[valid_mask]
                acc_mclass = (y_pred_3_valid == y_true_3_valid).mean()

                # 重新计算多分类混淆矩阵和指标
                cm_multi = confusion_matrix_3cls(y_true_3_valid, y_pred_3_valid, n_class=N_CLASSES)
                per_prec_multi, per_rec_multi, per_f1_multi, support_multi, weighted_f1_multi = metrics_from_cm(cm_multi)

            # ---------------- 3. 打印指标（调整精度到.6f） ----------------
            print(f"binary accuracy = {acc_bin:.6f}")
            print(f"mclass accuracy = {acc_mclass:.6f}")

            # Binary output
            print("== Binary output ==")
            print(f"weighted f1 score = {weighted_f1_bin:.6f}")
            bin_class_names = ["ok", "defect"]
            for c in range(2):
                name = bin_class_names[c]
                n_c = int(support_bin[c])
                prec_c = per_prec_bin[c]
                rec_c = per_rec_bin[c]
                f1_c = per_f1_bin[c]
                omission = 1.0 - rec_c if not np.isnan(rec_c) else 0.0

                print(
                    f"{name} (n={n_c}): "
                    f"precision:{prec_c:.6f}, "
                    f"recall:{rec_c:.6f}, "
                    f"f1_score:{f1_c:.6f}, "
                    f"omission_rate:{omission:.6f}"
                )

            # Multiclass output
            print("== Multiclass output ==")
            print(f"weighted f1 score = {weighted_f1_multi:.6f}")
            for c in range(N_CLASSES):
                name = CLASS_ORDER[c]
                n_c = int(support_multi[c]) if not np.isnan(support_multi[c]) else 0
                prec_c = per_prec_multi[c]
                rec_c = per_rec_multi[c]
                f1_c = per_f1_multi[c]
                omission = 1.0 - rec_c if not np.isnan(rec_c) else 0.0

                print(
                    f"{name} (n={n_c}): "
                    f"precision:{prec_c:.6f}, "
                    f"recall:{rec_c:.6f}, "
                    f"f1_score:{f1_c:.6f}, "
                    f"omission_rate:{omission:.6f}"
                )

        # ===== Binary calibration =====
        # (1) overall（二分类 top1）
        y_pred_bin_ece = (p_defect >= 0.5).astype(np.int64)
        conf_bin_pred = np.where(y_pred_bin_ece == 1, p_defect, p_ok)
        correct_bin = (y_pred_bin_ece == y_true_bin).astype(np.int64)

        ece_bin_overall, bin_edges_b, _, bin_acc_b, _ = compute_ece(
            conf_bin_pred, correct_bin, n_bins=N_BINS_ECE
        )

        bin_overall_path = os.path.join(
            save_dir, f"singlepad_binary_reliability_overall_{method_name}.png"
        )
        plot_reliability_bars(
            bin_edges_b,
            bin_acc_b,
            title=f"Binary reliability overall (top1, {method_name})",
            out_path=bin_overall_path,
        )

        # (2) classwise（true=ok / true=defect，prob-wise）和 deepensemble 一致
        conf_ok = p_ok
        corr_ok = (y_true_bin == 0).astype(np.int64)
        ece_ok, bin_edges_ok, _, bin_acc_ok, _ = compute_ece(
            conf_ok, corr_ok, n_bins=N_BINS_ECE
        )
        path_ok = os.path.join(
            save_dir,
            f"singlepad_binary_reliability_true_ok_{method_name}.png",
        )
        plot_reliability_bars(
            bin_edges_ok,
            bin_acc_ok,
            title=f"Binary reliability (true=ok, {method_name})",
            out_path=path_ok,
        )

        conf_def = p_defect
        corr_def = (y_true_bin == 1).astype(np.int64)
        ece_def, bin_edges_def, _, bin_acc_def, _ = compute_ece(
            conf_def, corr_def, n_bins=N_BINS_ECE
        )
        path_def = os.path.join(
            save_dir,
            f"singlepad_binary_reliability_true_defect_{method_name}.png",
        )
        plot_reliability_bars(
            bin_edges_def,
            bin_acc_def,
            title=f"Binary reliability (true=defect, {method_name})",
            out_path=path_def,
        )

        print(f"\n[{method_name}] Binary calibration")
        print(f"  overall ECE (top1) = {ece_bin_overall:.4f}")
        print(
            "  classwise ECE (prob-wise, by true class): "
            f"ok={ece_ok:.4f}, defect={ece_def:.4f}"
        )

        # ===== Multiclass calibration：top1 + prob-wise（true-class） =====
        # 先计算全量样本的多分类预测（阈值0.5）
        max_probs_multi_all = np.max(probs, axis=1)
        y_pred_3_all = np.where(max_probs_multi_all >= 0.5, probs.argmax(axis=1), -1)
        valid_mask_all = y_pred_3_all != -1
        
        # 过滤无效样本（预测为未知的）
        if np.sum(valid_mask_all) == 0:
            print("[WARN] no valid multiclass predictions for calibration, skip")
        else:
            y_true_3_valid_all = y_true_3[valid_mask_all]
            y_pred_3_valid_all = y_pred_3_all[valid_mask_all]
            probs_valid_all = probs[valid_mask_all]
            
            # (1) overall（三分类 top1）
            conf_multi_pred = probs_valid_all[np.arange(len(probs_valid_all)), y_pred_3_valid_all]
            correct_multi = (y_pred_3_valid_all == y_true_3_valid_all).astype(np.int64)

            ece_multi_overall, bin_edges_m, _, bin_acc_m, _ = compute_ece(
                conf_multi_pred, correct_multi, n_bins=N_BINS_ECE
            )

            multi_overall_path = os.path.join(
                save_dir, f"singlepad_multiclass_reliability_overall_{method_name}.png"
            )
            plot_reliability_bars(
                bin_edges_m,
                bin_acc_m,
                title=f"Multiclass reliability overall (top1, {method_name})",
                out_path=multi_overall_path,
            )

            # (2) classwise（true=class，prob-wise，每个样本只看 true class 概率）
            ece_multi_class = {}
            for c, cname in enumerate(CLASS_ORDER):
                conf_c = probs[:, c]
                corr_c = (y_true_3 == c).astype(np.int64)

                ece_c, bin_edges_c, _, bin_acc_c, _ = compute_ece(
                    conf_c, corr_c, n_bins=N_BINS_ECE
                )
                ece_multi_class[c] = ece_c

                path_c = os.path.join(
                    save_dir,
                    f"singlepad_multiclass_reliability_true_{cname}_{method_name}.png",
                )
                plot_reliability_bars(
                    bin_edges_c,
                    bin_acc_c,
                    title=f"Multiclass reliability (true={cname}, {method_name})",
                    out_path=path_c,
                )

            print(f"\n[{method_name}] Multiclass calibration (true-class prob-wise)")
            print(f"  overall ECE (top1) = {ece_multi_overall:.4f}")
            print(
                "  classwise ECE (prob-wise, by true class): "
                f"ok={ece_multi_class.get(0, float('nan')):.4f}, "
                f"undersolder={ece_multi_class.get(1, float('nan')):.4f}, "
                f"pseudosolder={ece_multi_class.get(2, float('nan')):.4f}"
            )

            # ===== Multiclass “向量版” ECE（每个样本所有类置信度都考虑） =====
            (
                ece_vec,
                mce_vec,
                bin_edges_vec,
                bin_counts_vec,
                gap_per_bin,
                acc_means_vec,
                conf_means_vec,
            ) = compute_multiclass_vector_ece(
                probs, y_true_3, n_bins=N_BINS_ECE
            )

            print(f"\n[{method_name}] Multiclass vector calibration (all-class probs)")
            print(f"  vector-ECE = {ece_vec:.4f}")
            print(f"  vector-MCE = {mce_vec:.4f}")

            # bin 级 gap 柱形图
            gap_path = os.path.join(
                save_dir,
                f"singlepad_multiclass_vector_gap_{method_name}.png",
            )
            plot_multiclass_vector_gap(
                bin_edges_vec,
                gap_per_bin,
                title=f"Multiclass vector gap per bin ({method_name})",
                out_path=gap_path,
            )

            # 按类别的向量版可靠性图：ok / undersolder / pseudosolder 各一张
            for c, cname in enumerate(CLASS_ORDER):
                out_c = os.path.join(
                    save_dir,
                    f"singlepad_multiclass_vector_reliability_{cname}_{method_name}.png",
                )
                plot_multiclass_vector_classwise(
                    bin_edges_vec,
                    acc_means_vec,
                    conf_means_vec,
                    class_idx=c,
                    class_name=cname,
                    title_prefix="Multiclass vector reliability",
                    out_path=out_c,
                )

    # === 3) 画二分类 F1 曲线 ===
    if not f1_curves:
        print("[WARN] no method produced F1 curves, skip plotting.")
        return

    plt.figure(figsize=(8, 5))

    for method_name, info in f1_curves.items():
        f1s = info["f1"]
        color = info["color"]

        best_idx = int(f1s.argmax())
        best_thr = thresholds_curve[best_idx]
        best_f1 = f1s[best_idx]

        print(f"\n[{method_name}] F1 curve (binary defect vs ok)")
        print(f"  Max F1 = {best_f1:.4f} at threshold = {best_thr:.3f}")

        plt.plot(
            thresholds_curve,
            f1s,
            label=f"{method_name} (max F1={best_f1:.3f} @ {best_thr:.2f})",
            color=color,
        )

    plt.xlabel("Threshold on P(defect)")
    plt.ylabel("F1 score (defect vs ok)")
    plt.title("singlepad F1 curve vs threshold (Laplace)")
    plt.xlim(0.0, 1.0)
    plt.ylim(0.0, 1.0)
    plt.grid(True, linestyle="--", alpha=0.3)
    plt.legend(loc="best")

    out_fig = os.path.join(save_dir, "singlepad_f1_curve_defect_vs_ok_laplace.png")
    plt.tight_layout()
    plt.savefig(out_fig, dpi=200, bbox_inches="tight")
    plt.close()
    print(f"\nF1 curve saved to: {out_fig}")


# ==============================
# main：整体流程
# ==============================

if __name__ == "__main__":
    part_name = "singlepad"

    # 1) 构建并加载 singlepad 模型
    model = _build_model_for_part(part_name)

    # 2) 基于 CSV 软标签拟合 Laplace（只对 BOM 最后一层）
    bom_layer, laplace_state = fit_laplace_last_layer_bom(
        model=model,
        csv_path=CSV_PATH,
        root_path=ROOT_DIR,
        part_name=part_name,
        batch_size=64,
        prior_precision=1.0,
        fisher_scale=1.0,
    )

    # 3) 用 Laplace 对所有样本做推理
    result_df = infer_singlepad_batch_laplace(
        csv_path=CSV_PATH,
        root_path=ROOT_DIR,
        model=model,
        bom_layer=bom_layer,
        laplace_state=laplace_state,
        part_name=part_name,
        batch_size=64,
        mc_samples=50,
    )

    print(f"result df length: {len(result_df)}")

    # 4) 按 id 对齐，把 Laplace 结果写回原 CSV
    orig_df = pd.read_csv(CSV_PATH)
    print(f"origin df length: {len(orig_df)}")

    dup_id = orig_df["id"].duplicated().sum()
    if dup_id > 0:
        raise ValueError(f"duplicate id: {dup_id} rows, please clean first")

    orig_df = orig_df.set_index("id")
    result_df = result_df.set_index("id")

    cols = [
        "NONE_CONF_LAPLACE",
        "INSUFFICIENT_SOLDER_CONF_LAPLACE",
        "PSEUDO_SOLDER_CONF_LAPLACE",
        "NONE_CONF_LAPLACE_STD",
        "INSUFFICIENT_SOLDER_CONF_LAPLACE_STD",
        "PSEUDO_SOLDER_CONF_LAPLACE_STD",
    ]

    orig_df[cols] = result_df[cols]

    merged = orig_df.reset_index()
    merged.to_csv(CSV_PATH, index=False, encoding="utf-8-sig")
    print("Laplace outputs written back to:", CSV_PATH)

    # 5) 指标 + ECE + 各类可靠性图 + 向量版 ECE + F1 曲线
    eval_singlepad_by_threshold(
        csv_path=CSV_PATH,
        save_dir=SAVE_DIR,
        part_name=part_name,
    )