...
def set_dropout_p(model: torch.nn.Module, p: float):
    ...
    for m in model.modules():
        if isinstance(m, (nn.Dropout, nn.Dropout2d, nn.Dropout3d)):
            m.p = p
            cnt += 1
    print(f"[INFO] set {cnt} Dropout layers p={p}")

@torch.no_grad()
def mc_predict_probs(model: torch.nn.Module, x1: torch.Tensor, x2: torch.Tensor, mc_times: int):
    model.eval()
    ...
    for _ in range(mc_times):
        ...
        #温度缩放
        # logits_bom = logits_bom/ 1.2

        # class_temps_tensor = torch.tensor([1.6,1.5,1], device=logits_bom.device)
        # class_temps_expanded = class_temps_tensor.view(*([1]*(logits_bom.dim()-1)), -1)
        # logits_bom = logits_bom / class_temps_expanded
        probs = F.softmax(logits_bom, dim=-1)
        probs_list.append(probs)
    arr = torch.stack(probs_list, dim=0)  # [T,B,C]
    return arr.mean(0), arr.std(0)

def infer_black_mcdropout(df_black: pd.DataFrame, model: torch.nn.Module, mc_times: int, batch_size: int):
    N = len(df_black)
    means, stds = [], []

    with tqdm(total=N, desc="MC Dropout infer (black)", unit="img") as pbar:
        for start in range(0, N, batch_size):
            ...
            m, s = mc_predict_probs(model, x1, x2, mc_times=mc_times)
...
def eval_one_split(df: pd.DataFrame, split_name: str, out_dir: str,
    ...
    #  修复“缝隙不均匀/重叠”：柱子 x 用 bin center（等间距）
    # =========================
    ...
    # =========================
    # (D) Vector gap bars (hard): overall + 3 classwise（4张）
    # =========================
    ...
    # =========================
    # (E) Soft gap bars + count (abs_top1 + l1)
    # =========================
    ...
    # =========================
    # 直接统计 gap 区间样本数量（并导出 CSV）
    #   - abs_top1: [0,1]
    #   - L1 gap (3类): 约 [0,2]
    #   bins 数量用 N_BINS_ECE，和你 ECE 的 bins 数保持一致
    # =========================
    hist_edges_abs = np.linspace(0.0, 1.0, N_BINS_ECE + 1, dtype=np.float64)
    plot_gap_hist(
        soft_stats["gap_abs_top1"],
        title=f"[{split_name}] Soft ABS(top1) gap histogram (N_BINS={N_BINS_ECE})",
    ...
    plot_gap_hist(
        soft_stats["gap_l1"],
        title=f"[{split_name}] Soft L1 gap histogram (N_BINS={N_BINS_ECE})",
...
# =========================
# main: black only + MC Dropout overwrite + eval/plot
# =========================
def main():
    ensure_dir(SAVE_DIR)
    ...
    for p in DROPOUT_P_LIST:
        tag = f"p{p:.2f}".replace(".", "_") + f"_mc{MC_TIMES}"
        run_dir = os.path.join(SAVE_DIR, tag)
        ensure_dir(run_dir)

        print("\n" + "=" * 90)
        print(f"[RUN {tag}] MC Dropout infer black only (C=3)")
        print("=" * 90)

        set_dropout_p(model, p)
        mean3, std3 = infer_black_mcdropout(df_black, model, mc_times=MC_TIMES, batch_size=BATCH_SIZE)
        ...
        out_csv = os.path.join(run_dir, f"black_mcdropout_overwrite_{tag}.csv")
